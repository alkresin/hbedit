...
STATIC FUNCTION hbc_FCopyFile( aDir, cFileTo, nStart, aWnd )
   ...
   IF lSilent .OR. !Empty( cFileTo := FAsk_Copy( ;
      ...
      IF lDir
         ...
         ENDIF
         hbc_Wndclose( aWnd, _I("Done")+", " + Ltrim(Str(nCopied)) + " "+_I("files copied"), .T. )
         IF !lSilent
            oPaneTo:Refresh()
            oPaneCurr:Refresh()
            oPaneCurr:RedrawAll()
      ...
      ELSE
         IF !lSilent .AND. nStart == 0 .AND. Empty( aWnd )
            ...
            hbc_Wndout( aWnd, "" )
         ENDIF
         IF ( nRes := FCopy( aDir, cFileTo, nStart, aWnd ) ) < 2
            lRes := .T.
...
STATIC FUNCTION hbc_FDelete( lSilent, cFileName, lDir )
   ...
   IF lSilent .OR. edi_Alert( _I("Really delete")+" " + FTransl(cFileName) + "?", _I("Yes"), _I("No") ) == 1
      ...
      IF nRes == Nil
         IF lDir
            lRes := .T.
            aWnd := hbc_Wndinit( 05, oPaneCurr:vx1+12, 12, oPaneCurr:vx2-12,, _I("Delete") )
...
STATIC FUNCTION hbc_Zip()
   ...
   IF ( nRes := edi_READ( aGets ) ) > 0 .AND. nRes < Len(aGets) .AND. !Empty(aGets[2,4])
      ...
      Restscreen( 05, 10, 10, 70, cScBuf )
      IF !Empty( oPaneCurr:aSelected )
         aWnd := hbc_Wndinit( 05, oPaneCurr:vx1+12, 16, oPaneCurr:vx2-12,, "Zip" )
         FOR i := 1 TO Len( oPaneCurr:aSelected )
...
STATIC FUNCTION hbc_Unzip()
   ...
   IF ( nRes := edi_READ( aGets ) ) > 0 .AND. nRes < Len(aGets)
      ...
      IF cExt == ".zip"
         ...
         IF ! Empty( hUnzip )
            aWnd := hbc_Wndinit( 05, oPaneCurr:vx1+12, 16, oPaneCurr:vx2-12,, "Unzip" )
            nErr := hb_unzipFileFirst( hUnzip )
            DO WHILE nErr == 0
               hb_unzipFileInfo( hUnzip, @cFile, @dDate, @cTime, , , , @nSize, @lCrypted )
               hbc_Wndout( aWnd, cFile )
...
FUNCTION hbc_Wndout( arr, cText, lOver )
...
FUNCTION hbc_Wndclose( arr, cText, lOver )
