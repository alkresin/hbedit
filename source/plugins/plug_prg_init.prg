   ...
   LOCAL bStartEdit := {|o|
      ...
      IF o:lTopPane
         ...
         oEdit:hCargo["help"] := "Harbour plugin hotkeys:" + Chr(10) + ;
            ...
            "  Alt-R  - Run, Compile, Preprocess" + Chr(10) + ;
...
STATIC FUNCTION _prg_RC( oEdit )

   LOCAL aMenu := { "Run", "Compile to hrb", "Preprocess" }, i
...
STATIC FUNCTION _prg_Prepro( oEdit )

   LOCAL acmd, i, ie, cTemp, cFile := "$hb_compile_err", oNew
   LOCAL cFileRes := hb_DirTemp() + "hb_compile_err.out", cFilePrg := hb_DirTemp() + "hbcompile.prg"
   LOCAL nRow := Row(), nCol := Col()

   edi_CloseWindow( cFile )

   acmd := { "harbour", cFilePrg, "-n2", "-q", "-w", "-p", "-o"+hb_fnameExtSet(cFilePrg,".c") }
   IF !Empty( cPathInclude )
      AAdd( aCmd, "-I" + cPathInclude )
   ENDIF

   hb_MemoWrit( cFilePrg, oEdit:ToString() )
   i := cedi_rediron( 1, hb_DirTemp() + "hbcompile.out" )
   ie := cedi_rediron( 2, cFileRes )
   hb_compile( hb_ArrayToParams( acmd ) )
   cedi_rediroff( 2, ie )
   cedi_rediroff( 1, i )
   CLEAR SCREEN

   cTemp := Memoread( cFileRes )
   IF !Empty( cTemp ) .AND. " Warning " $ cTemp .AND. ;
   ...
   ELSE
      oNew := mnu_NewBuf( oEdit, "$Preprocessor", Memoread( hb_fnameExtSet(cFilePrg,".ppo") ) )
   ENDIF
...
STATIC FUNCTION _prg_Compile( oEdit )

   LOCAL acmd, cHrb, i, ie, cName := "", cTemp, nPos, cFile := "$hb_compile_err", oNew
...
STATIC FUNCTION SaveHrb( oEdit, cHrb, cName )
   ...
   LOCAL aGets := { {11,22,0,"",33,oEdit:cColorSel,oEdit:cColorMenu}, ;
      {12,29,0,"",26,oEdit:cColorSel,oEdit:cColorMenu}, {13,23,1,.F.,1}, ;
      {15,25,2,"[Save]",8,oEdit:cColorSel,oEdit:cColorMenu,{||__KeyBoard(Chr(13))}}, ;
      {15,40,2,"[Cancel]",10,oEdit:cColorSel,oEdit:cColorMenu,{||__KeyBoard(Chr(27))}} }
   LOCAL nRes, arr, i, s := "", cEol, cFull, cTitle
...
STATIC FUNCTION _prg_Run( oEdit )
...
STATIC FUNCTION _prg_Err_OnKey( oEdit, nKeyExt )
